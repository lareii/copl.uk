...
func SetupRouter(app *fiber.App) {
	...
	userGroup.Get("/:slug", middlewares.AuthMiddleware(), middlewares.RateLimiterMiddleware(5, 60), users.GetUser)
	userGroup.Get("/:slug/follows", middlewares.AuthMiddleware(), users.Follows)
	userGroup.Post("/:slug/follows", middlewares.AuthMiddleware(), middlewares.RateLimiterMiddleware(10, 60), users.FollowUser)
	userGroup.Get("/:slug/posts", middlewares.AuthMiddleware(), middlewares.RateLimiterMiddleware(10, 60), users.GetUserPosts)
